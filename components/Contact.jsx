export default function Contact({ dessert, cartItems, onAdd, onRemove }) {
  const { image, name, category, price, id } = dessert;
;  

  // Find the item in the cart, if exists
  const existingCartItem = cartItems.find(item => item.id === id);
  const quantity = existingCartItem ? existingCartItem.quantity : 0;

  // Add one more to cart
  function handleAdd() {
    onAdd(dessert);
  }





function handleRemove() {
  if (quantity > 1) {
    onAdd(dessert, true); // pass decrement = true
  } else if (quantity === 1) {
    onRemove(id);
  }
}

  return (
    <article className="dessert-card">
      <div className="main_container_dessert">
        <div className="img_dessert_container">
          <picture>
            <source media="(min-width:1024px)" srcSet={image.desktop} />
            <source media="(min-width:768px)" srcSet={image.tablet} />
            <source media="(max-width:767px)" srcSet={image.mobile} />
            <img className="image_dessert" src={image.desktop} alt={name} />
          </picture>

          <div className="addtoCartContainer addedtoCart">
            {quantity === 0 ? (
              <button className="addtoCart" onClick={handleAdd}>
                <img
                  src="./assets/images/icon-add-to-cart.svg"
                  alt="Add to cart icon"
                />
                <span>Add to Cart</span>
              </button>
            ) : (
              <div className="counter_container "
                style={{
                  backgroundColor: "var(--Red)",
                  border: "solid var(--Red) 0.12rem",
                  color: "var(--Rose-50)",
                  fontWeight: "var(--Weight-400)",
                  justifyContent: "space-around",
                  height: "2.823125rem",
                  display: "flex",
                  alignItems: "center",
                  gap: "8px",
                }}
              >
                <button className="decrement" onClick={handleRemove}>
                  -
                </button>
                <span>{quantity}</span>
                <button className="increment" onClick={handleAdd}>
                  +
                </button>
              </div>
            )}
          </div>
        </div>

        <div className="product_info">
          <p className="category">{category}</p>
          <p className="name_product">{name}</p>
          <p className="price">{price}</p>
        </div>
      </div>
    </article>
  );
}